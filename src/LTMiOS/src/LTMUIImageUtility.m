...
@implementation LTMUIImageUtility
...
+ (UIImage *)resizeImage:(UIImage *)image toSize:(CGSize)size contentMode:(UIViewContentMode)contentMode{
    ...
    if (size.width <= 0 || size.height <= 0) return nil;
    UIGraphicsBeginImageContextWithOptions(size, NO, image.scale);
    [LTMUIImageUtility drawImage:image inRect:CGRectMake(0, 0, size.width, size.height) withContentMode:contentMode clipsToBounds:NO];
    UIImage *resizeImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return resizeImage;
}
...
+ (UIImage *)cutImage:(UIImage *)image roundCornerRadius:(CGFloat)radius{
    ...
    return  [LTMUIImageUtility cutImage:image roundCornerRadius:radius corners:UIRectCornerAllCorners borderWidth:0];
}
...
+ (UIImage *)rotateImageLeft90:(UIImage *)image{
    ...
    return [LTMUIImageUtility rotateImage:image radians:DegreesToRadians(90) fitSize:YES];
}
...
+ (UIImage *)RotateImage180:(UIImage *)image{
    ...
    return [LTMUIImageUtility  flipImage:image horizontal:YES vertical:YES];
}
...
+ (UIImage *)flipImageVertical:(UIImage *)image{
    ...
    return [LTMUIImageUtility flipImage:image horizontal:NO vertical:YES];
}

+ (UIImage *)flipImageHorizontal:(UIImage *)image{
    
    return [LTMUIImageUtility flipImage:image horizontal:YES vertical:NO];
}
