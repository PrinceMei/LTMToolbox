...
#import "LTMGIFImage.h"
#import <ImageIO/ImageIO.h>
#import <MobileCoreServices/MobileCoreServices.h>
#import "LTMNSStringUtility.h"
#import "LTMUIImageUtility.h"

@implementation LTMGIFImage {
...
+ (instancetype)imageNamed:(NSString *)name {
    ...
    for (int s = 0; s < scales.count; s++) {
        ...
        NSString *scaledName = [LTMNSStringUtility filenameWithString:res appendingByNameScale:scale];
...
- (instancetype)initWithContentsOfFile:(NSString *)path {
    NSData *data = [NSData dataWithContentsOfFile:path];
    return [self initWithData:data scale:[LTMNSStringUtility pathScaleWithString:path]];
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"LFGIFScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"LFGIFData"];
    if (data) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    if (_gifData) {
        [aCoder encodeObject:@(self.scale) forKey:@"LFGIFScale"];
        [aCoder encodeObject:_gifData forKey:@"LFGIFData"];
    }
}

#pragma mark - LFAnimtedImage
