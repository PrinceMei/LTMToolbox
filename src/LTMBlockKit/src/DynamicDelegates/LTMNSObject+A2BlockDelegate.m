...
static inline LTMA2DynamicDelegate *getDynamicDelegate(NSObject *delegatingObject, Protocol *protocol, const A2BlockDelegateInfo *info, BOOL ensuring) {
	LTMA2DynamicDelegate *dynamicDelegate = [delegatingObject ltm_dynamicDelegateForProtocol:a2_protocolForDelegatingObject(delegatingObject, protocol)];
...
{
	[self ltm_linkProtocol:a2_dataSourceProtocol(self) methods:dictionary];
}

+ (void)ltm_linkDelegateMethods:(NSDictionary *)dictionary
{
	[self ltm_linkProtocol:a2_delegateProtocol(self) methods:dictionary];
}
...
{
	[self ltm_registerDynamicDelegateNamed:@"dataSource" forProtocol:a2_dataSourceProtocol(self)];
}
+ (void)ltm_registerDynamicDelegate
{
	[self ltm_registerDynamicDelegateNamed:@"delegate" forProtocol:a2_delegateProtocol(self)];
}

+ (void)ltm_registerDynamicDataSourceNamed:(NSString *)dataSourceName
{
	[self ltm_registerDynamicDelegateNamed:dataSourceName forProtocol:a2_dataSourceProtocol(self)];
}
+ (void)ltm_registerDynamicDelegateNamed:(NSString *)delegateName
{
	[self ltm_registerDynamicDelegateNamed:delegateName forProtocol:a2_delegateProtocol(self)];
}
...
+ (void)ltm_registerDynamicDelegateNamed:(NSString *)delegateName forProtocol:(Protocol *)protocol
{
	...
	if (![self instancesRespondToSelector:getter]) {
		IMP getterImplementation = imp_implementationWithBlock(^(NSObject *delegatingObject) {
			return [delegatingObject ltm_dynamicDelegateForProtocol:a2_protocolForDelegatingObject(delegatingObject, protocol)];
		});
