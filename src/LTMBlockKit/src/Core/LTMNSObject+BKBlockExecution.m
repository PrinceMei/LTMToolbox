...
#import "LTMNSObject+BKBlockExecution.h"
...
{
    return [self ltm_performOnQueue:BKGetBackgroundQueue() afterDelay:delay usingBlock:block];
}

+ (id <NSObject, NSCopying>)ltm_performInBackgroundAfterDelay:(NSTimeInterval)delay usingBlock:(void (^)(void))block
{
    return [NSObject ltm_performOnQueue:BKGetBackgroundQueue() afterDelay:delay usingBlock:block];
}
...
@implementation NSObject (LTMNSObjectBlockExecutionAdditions_Deprecated)

#pragma mark - Legacy verions

- (id <NSObject, NSCopying>)ltm_performBlock:(void (^)(id obj))block afterDelay:(NSTimeInterval)delay
{
    return [self ltm_performAfterDelay:delay usingBlock:block];
}

+ (id <NSObject, NSCopying>)ltm_performBlock:(void (^)(void))block afterDelay:(NSTimeInterval)delay
{
    return [self ltm_performAfterDelay:delay usingBlock:block];
}

- (id <NSObject, NSCopying>)ltm_performBlockInBackground:(void (^)(id obj))block afterDelay:(NSTimeInterval)delay
{
    return [self ltm_performInBackgroundAfterDelay:delay usingBlock:block];
}

+ (id <NSObject, NSCopying>)ltm_performBlockInBackground:(void (^)(void))block afterDelay:(NSTimeInterval)delay
{
    return [self ltm_performInBackgroundAfterDelay:delay usingBlock:block];
}

- (id <NSObject, NSCopying>)ltm_performBlock:(void (^)(id obj))block onQueue:(dispatch_queue_t)queue afterDelay:(NSTimeInterval)delay
{
    return [self ltm_performOnQueue:queue afterDelay:delay usingBlock:block];
}

+ (id <NSObject, NSCopying>)ltm_performBlock:(void (^)(void))block onQueue:(dispatch_queue_t)queue afterDelay:(NSTimeInterval)delay
{
    return [self ltm_performOnQueue:queue afterDelay:delay usingBlock:block];
}
