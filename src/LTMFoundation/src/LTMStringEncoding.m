...
#ifndef _DKSEDevLog
#ifdef DEBUG
#define _DKSEDevLog(...) NSLog(__VA_ARGS__)
#else
#define _DKSEDevLog(...) do { } while (0)
#endif
#endif
    ...
    if (self = [super init]) {
        ...
        for (unsigned int i = 0; i < length; i++) {
            if (_reverseCharMap[(int)_charMap[i]] != kUnknownChar) {
                _DKSEDevLog(@"Duplicate character at pos %d",i);
                return nil;
            }
...
- (NSData *)decode:(NSString *)inString {
    ...
    if (!inBuf) {
        _DKSEDevLog(@"unable to convert buffer to ASCII");
        return nil;
    }
    ...
    for (NSUInteger i = 0; i < inLen; i++) {
        ...
        switch (val) {
            ...
            case kPaddingChar:
                ...
                break;
            case kUnknownChar:
                _DKSEDevLog(@"Unexpected data in input pos %lu", (unsigned long)i);
                return nil;
            default:
                if (expectPad) {
                    _DKSEDevLog(@"Expected further padding characters");
                    return nil;
                }
                buffer <<= _shift;
                buffer |= val & _mask;
    ...
    if (bitsLeft && buffer & ((1 << bitsLeft) - 1)) {
        _DKSEDevLog(@"Incomplete trailing data");
        return nil;
    }
