...
NSString *const LTMA2IncompatibleMethodSignatureKey = @"incompatibleMethodSignature";

#pragma mark Block Internals

typedef NS_OPTIONS(int, LTMBlockFlags) {
	LTMBlockFlagsHasCopyDisposeHelpers = (1 << 25),
	LTMBlockFlagsHasSignature          = (1 << 30)
};
...
typedef struct _BKBlock {
	...
	struct {
		...
		// requires LTMBlockFlagsHasCopyDisposeHelpers
		void (*copy)(void *dst, const void *src);
		void (*dispose)(const void *);
		// requires LTMBlockFlagsHasSignature
...
@interface LTMA2BlockInvocation ()

@property (nonatomic, readonly) NSMethodSignature *blockSignature;

@end

@implementation LTMA2BlockInvocation
...
+ (NSMethodSignature *)typeSignatureForBlock:(id)block __attribute__((pure, nonnull(1)))
{
	...
	if (!(layout->flags & LTMBlockFlagsHasSignature))
		return nil;

	void *desc = layout->descriptor;
	desc += 2 * sizeof(unsigned long int);

	if (layout->flags & LTMBlockFlagsHasCopyDisposeHelpers)
		desc += 2 * sizeof(void *);
...
- (instancetype)initWithBlock:(id)block methodSignature:(NSMethodSignature *)methodSignature
{
	...
	if (![[self class] isSignature:methodSignature compatibleWithSignature:blockSignature]) {
		@throw [NSException exceptionWithName:NSInvalidArgumentException reason:@"Attempted to create block invocation with incompatible signatures" userInfo:@{LTMA2IncompatibleMethodSignatureKey: methodSignature}];
	}
	return (self = [self initWithBlock:block methodSignature:methodSignature blockSignature:blockSignature]);
}
