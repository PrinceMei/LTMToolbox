...
static BOOL ltm_object_isKindOfClass(id obj, Class testClass)
{
...
static Protocol *ltm_a2_protocolForDelegatingObject(id obj, Protocol *protocol)
{
...
static inline LTMA2DynamicDelegate *getDynamicDelegate(NSObject *delegatingObject, Protocol *protocol, const A2BlockDelegateInfo *info, BOOL ensuring) {
	LTMA2DynamicDelegate *dynamicDelegate = [delegatingObject ltm_dynamicDelegateForProtocol:ltm_a2_protocolForDelegatingObject(delegatingObject, protocol)];
	...
	if (ltm_object_isKindOfClass(originalDelegate, LTMA2DynamicDelegate.class)) { return dynamicDelegate; }
...
typedef LTMA2DynamicDelegate *(^A2GetDynamicDelegateBlock)(NSObject *, BOOL);
...
@implementation NSObject (LTMNSObjectA2BlockDelegateAdditions)

#pragma mark Helpers

+ (NSMapTable *)ltm_delegateInfoByProtocol:(BOOL)createIfNeeded
{
...
+ (const A2BlockDelegateInfo *)ltm_delegateInfoForProtocol:(Protocol *)protocol
{
	A2BlockDelegateInfo *infoAsPtr = NULL;
	Class cls = self;
	while ((infoAsPtr == NULL || infoAsPtr->getter == NULL) && cls != nil && cls != NSObject.class) {
		NSMapTable *map = [cls ltm_delegateInfoByProtocol:NO];
...
+ (void)ltm_linkDataSourceMethods:(NSDictionary *)dictionary
{
	[self ltm_linkProtocol:ltm_a2_dataSourceProtocol(self) methods:dictionary];
}

+ (void)ltm_linkDelegateMethods:(NSDictionary *)dictionary
{
	[self ltm_linkProtocol:ltm_a2_delegateProtocol(self) methods:dictionary];
}

+ (void)ltm_linkProtocol:(Protocol *)protocol methods:(NSDictionary *)dictionary
{
	[dictionary enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *selectorName, BOOL *stop) {
		...
		const A2BlockDelegateInfo *info = [self ltm_delegateInfoForProtocol:protocol];

		IMP getterImplementation = imp_implementationWithBlock(^(NSObject *delegatingObject) {
			LTMA2DynamicDelegate *delegate = getDynamicDelegate(delegatingObject, protocol, info, NO);
		...
		IMP setterImplementation = imp_implementationWithBlock(^(NSObject *delegatingObject, id block) {
			LTMA2DynamicDelegate *delegate = getDynamicDelegate(delegatingObject, protocol, info, YES);
...
+ (void)ltm_registerDynamicDataSource
{
	[self ltm_registerDynamicDelegateNamed:@"dataSource" forProtocol:ltm_a2_dataSourceProtocol(self)];
}
+ (void)ltm_registerDynamicDelegate
{
	[self ltm_registerDynamicDelegateNamed:@"delegate" forProtocol:ltm_a2_delegateProtocol(self)];
}

+ (void)ltm_registerDynamicDataSourceNamed:(NSString *)dataSourceName
{
	[self ltm_registerDynamicDelegateNamed:dataSourceName forProtocol:ltm_a2_dataSourceProtocol(self)];
}
+ (void)ltm_registerDynamicDelegateNamed:(NSString *)delegateName
{
	[self ltm_registerDynamicDelegateNamed:delegateName forProtocol:ltm_a2_delegateProtocol(self)];
}

+ (void)ltm_registerDynamicDelegateNamed:(NSString *)delegateName forProtocol:(Protocol *)protocol
{
	NSMapTable *propertyMap = [self ltm_delegateInfoByProtocol:YES];
	...
	IMP setterImplementation = imp_implementationWithBlock(^(NSObject *delegatingObject, id delegate) {
		LTMA2DynamicDelegate *dynamicDelegate = getDynamicDelegate(delegatingObject, protocol, infoAsPtr, YES);
	...
	if (![self instancesRespondToSelector:getter]) {
		IMP getterImplementation = imp_implementationWithBlock(^(NSObject *delegatingObject) {
			return [delegatingObject ltm_dynamicDelegateForProtocol:ltm_a2_protocolForDelegatingObject(delegatingObject, protocol)];
		});
...
- (id)ltm_ensuredDynamicDelegate
{
	Protocol *protocol = ltm_a2_delegateProtocol(self.class);
	return [self ltm_ensuredDynamicDelegateForProtocol:protocol];
}

- (id)ltm_ensuredDynamicDelegateForProtocol:(Protocol *)protocol
{
	const A2BlockDelegateInfo *info = [self.class ltm_delegateInfoForProtocol:protocol];
