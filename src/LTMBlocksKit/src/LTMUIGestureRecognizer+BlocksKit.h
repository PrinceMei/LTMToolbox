//
//  LTMUIGestureRecognizer+BlocksKit.h
//  LTMBlocksKit
//
...
@interface UIGestureRecognizer (LTMUIGestureRecognizerBlocksAdditions)
...
/** Initializes an allocated gesture recognizer that will call the given block
 ...
 @param delay A number of seconds after which the block will fire.
 */
- (instancetype)ltm_initWithHandler:(void (^)(UIGestureRecognizer *sender, UIGestureRecognizerState state, CGPoint location))block delay:(NSTimeInterval)delay LTM_INITIALIZER;
...
/** Initializes an allocated gesture recognizer that will call the given block.
 ...
 @param block The block which handles an executed gesture.
 */
- (instancetype)ltm_initWithHandler:(void (^)(UIGestureRecognizer *sender, UIGestureRecognizerState state, CGPoint location))block LTM_INITIALIZER;

/** Allows the block that will be fired by the gesture recognizer
 to be modified after the fact.
 */
@property (nonatomic, copy, setter = ltm_setHandler:, nullable) void (^ltm_handler)(UIGestureRecognizer *sender, UIGestureRecognizerState state, CGPoint location);

/** Allows the length of the delay after which the gesture
 recognizer will be fired to modify. */
@property (nonatomic, setter = ltm_setHandlerDelay:) NSTimeInterval ltm_handlerDelay;
